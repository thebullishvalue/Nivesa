...
VERSION = "2.6.0"
BUILD = "2026.07.AMORTIZATION_FIDELITY"
PRODUCT_NAME = "Nivesa"
...
def get_positions_dataframe():
    ...
    for (bid, acct), grp in txns.groupby(['bond_id', 'account']):
        ...
        buy_units_sum = buy_dateord_sum = 0.0
        for _, tr in trades.iterrows():
            if tr['transaction_type'] == 'Buy':
                units_held += abs(tr['units'])
                cost_held  += tr['amount']
                buy_units_sum   += abs(tr['units'])
                buy_dateord_sum += abs(tr['units']) * tr['trade_date'].toordinal()
        ...
        # Units-weighted average purchase date — the anchor for a true
        # yield-to-cost (the IRR contracted at purchase, invariant to seasoning).
        wavg_buy_date = date.fromordinal(int(round(buy_dateord_sum / buy_units_sum))) if buy_units_sum > 0 else date.today()
